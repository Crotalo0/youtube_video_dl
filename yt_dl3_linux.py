...
def sec_converter(s):
    """Convert seconds in hour, min and sec."""
    h = s // 3600
    s = s - 3600 * h
    m = s // 60
    s = s - 60 * m
...
class Window(tk.Toplevel):
    """Tk window class for dl in progress."""
...
class Window2(tk.Toplevel):
    ...
    def create_widgets_dl(self):
        ...
        dl_label = ttk.Label(
            dl_frame, text="Merging in progress...\n\nThis could take a while, just wait.")
        dl_label.grid(row=1, column=1, pady=10, padx=60)
...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        def downloader():
            ...
            try:
                ...
                if video_dl == 1 and audio_dl == 0:
                    ...
                    dl_win.destroy()
                    messagebox.showinfo(
                        "Information", "Video downloaded successfully!")
                    ...
                    dl_win.destroy()
                    messagebox.showinfo(
                        "Information", "Audio downloaded successfully!")
                    ...
                    dl_win.destroy()
                    messagebox.showinfo(
                        "Information", "Audio and video downloaded successfully!")

                    final_path_video = str(
                        App.SAVE_PATH) + r'/video' + f'/{video_filename}'
                    final_path_audio = str(
                        App.SAVE_PATH) + r'/audio' + f'/{audio_filename}'
                    ...
                    messagebox.showinfo(
                        "Information", "Merging Audio and Video ended!\nYour file is ready.")
                ...
                if merging_win is not None:
                    ...
                    hour, minutes, seconds = sec_converter(end - start)
                    print(
                        f"...finished! In: {int(hour)}h, {int(minutes)}m, {int(seconds)}s.")
        ...
        m.add_command(
            label="Cut", command=lambda: url_entry.event_generate('<Control-x>'))
        m.add_command(
            label="Copy", command=lambda: url_entry.event_generate('<Control-c>'))
        m.add_command(
            label="Paste", command=lambda: url_entry.event_generate('<Control-v>'))
        m.add_separator()
        m.add_command(label='Highlight all',
                      command=lambda: url_entry.select_range(0, "end"))
        m.add_separator()
        ...
        info_label = ttk.Label(
            info_frame, text="", justify=tk.CENTER, font=("Calibri", 10, "italic"))
        ...
        sub1_frame = ttk.LabelFrame(
            main_frame, text=" Additional options ", relief=tk.RIDGE)
        ...
        # - - - - - - - - - - - - - - - - - - - - -
        # Button (sub2) frame
        sub2_frame = ttk.LabelFrame(
            main_frame, text=" Buttons ", relief=tk.RIDGE)
        sub2_frame.grid(row=2, column=2, sticky=tk.N + tk.S, padx=6)

        sub2_dir = ttk.Label(
            sub2_frame, text="Current save directory: ", font=("Calibri", 9, "italic"))
        sub2_dir.grid(row=1, column=1, pady=3, padx=3)

        sub2_path = ttk.Label(
            sub2_frame, text="Same folder as the program", font=("Calibri", 9, "italic"))
        sub2_path.grid(row=2, column=1, pady=3, padx=3)

        save_path = ttk.Button(
            sub2_frame, text="Set save path...", command=save_path)
        save_path.grid(row=3, column=1, pady=3, padx=58)

        video_info = ttk.Button(
            sub2_frame, text="Get video info", command=get_info)
        video_info.grid(row=4, column=1, pady=3, padx=58)

        dl_button = ttk.Button(
            sub2_frame, text="Start download!", command=downloader)
        dl_button.grid(row=5, column=1, sticky=tk.W,
                       pady=5, padx=5, ipady=5, ipadx=58)
